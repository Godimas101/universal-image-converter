...
{
    [MyTextSurfaceScript("LCDInfoScreenComponentsSummary", "$IOS LCD - Components")]
    public class LCDComponentsSummaryInfo : MyTextSurfaceScriptBase
    {
        MyIni config = new MyIni();

        public static string CONFIG_SECTION_ID = "SettingsComponentsSummary";
        ...
        void CreateConfig()
        {
            ...
            sb.AppendLine();
            sb.AppendLine("; [ COMPONENTS - SCREEN OPTIONS ]");
            sb.AppendLine($"showComponents={surfaceData.showComponents}");
            sb.AppendLine($"showProtoComponents={surfaceData.showProtoComponents}");
            sb.AppendLine($"UseSubtypeId={surfaceData.useSubtypeId}");

            sb.AppendLine();
            sb.AppendLine("; [ COMPONENTS - ITEM THRESHOLDS ]");
        ...
        void LoadConfig()
        {
            try
            {
                ...
                if (config.TryParse(myTerminalBlock.CustomData, CONFIG_SECTION_ID, out result))
                {
                    ...
                    // Optional category toggles (backward compatible)
                    if (config.ContainsKey(CONFIG_SECTION_ID, "showComponents"))
                        surfaceData.showComponents = config.Get(CONFIG_SECTION_ID, "showComponents").ToBoolean();
                    if (config.ContainsKey(CONFIG_SECTION_ID, "showProtoComponents"))
                        surfaceData.showProtoComponents = config.Get(CONFIG_SECTION_ID, "showProtoComponents").ToBoolean();
                ...
                {
                    MyLog.Default.WriteLine($"MahrianeIndustries.LCDInfo.LCDComponentsSummaryInfo: Config Syntax error at Line {result}");
                    configError = true;
                }
                ...
                // Check unknownComponents config
            ...
            {
                MyLog.Default.WriteLine($"MahrianeIndustries.LCDInfo.LCDComponentsSummaryInfo: Caught Exception while loading config: {e.ToString()}");
            }
        }
        ...
        public LCDComponentsSummaryInfo(IMyTextSurface surface, IMyCubeBlock block, Vector2 size) : base(surface, block, size)
        {
            mySurface = surface;
        ...
        void UpdateInventories()
        {
            ...
            {
                MyLog.Default.WriteLine($"MahrianeIndustries.LCDInfo.LCDComponentsSummaryInfo: Caught Exception while updating inventories: {e.ToString()}");
            }
        }
        ...
        void UpdateContents()
        {
            ...
            {
                MyLog.Default.WriteLine($"MahrianeIndustries.LCDInfo.LCDComponentsSummaryInfo: Caught Exception while updating contents: {e.ToString()}");
            }
        }
        ...
        void DrawMainSprite(ref MySpriteDrawFrame frame, ref Vector2 position)
        {
            try
            {
                ...
                {
                    SurfaceDrawer.DrawHeader(ref frame, ref position, surfaceData, $"Components [{cargo.Count}/{itemDefinitions.Count}/{unknownItemDefinitions.Count}/{inventories.Count}]:");
                ...
                if (surfaceData.showSummary)
                {
                    ...
                    volume /= 1000;
                    SurfaceDrawer.DrawBar(ref frame, ref position, surfaceData, "Comp. Volume", volume, current, Unit.Percent, false, true);
                    position += surfaceData.newLine;
                }
            ...
            {
                MyLog.Default.WriteLine($"MahrianeIndustries.LCDInfo.LCDComponentsSummaryInfo: Caught Exception while DrawMainSprite: {e.ToString()}");
            }
        }
        ...
        void DrawAllKnownSprite(ref MySpriteDrawFrame frame, ref Vector2 position)
        {
            try
            {
                ...
                string filterTag = $"[{(surfaceData.showComponents ? "C" : "*")}/{(surfaceData.showProtoComponents ? "P" : "*")}]";
                SurfaceDrawer.WriteTextSprite(ref frame, headerPosition, surfaceData, $"Id [Components]{filterTag}", TextAlignment.LEFT, surfaceData.surface.ScriptForegroundColor);
            ...
            {
                MyLog.Default.WriteLine($"MahrianeIndustries.LCDInfo.LCDComponentsSummaryInfo: Caught Exception while DrawAllKnownSprite: {e.ToString()}");
            }
        }
        ...
        void DrawAllAvailableSprite(ref MySpriteDrawFrame frame, ref Vector2 position)
        {
            try
            {
                if (cargo.Count <= 0)
                {
                    SurfaceDrawer.WriteTextSprite(ref frame, position, surfaceData, "No Components found.", TextAlignment.LEFT, surfaceData.surface.ScriptForegroundColor);
                    position += surfaceData.newLine;
                    return;
                }
            ...
            {
                MyLog.Default.WriteLine($"MahrianeIndustries.LCDInfo.LCDComponentsSummaryInfo: Caught Exception while DrawAllAvailableSprite: {e.ToString()}");
            }
        }
        ...
        bool IgnoreDefinition(CargoItemDefinition itemDefinition)
        {
            ...
            // Force-exclude specific component subtype IDs that should not appear on this screen
            if (string.Equals(itemDefinition.subtypeId, "ZoneChip", StringComparison.OrdinalIgnoreCase))
                return true;
        ...
        CargoItemDefinition FindCargoItemDefinition(string typeId, string subtypeId)
        {
            ...
            // Tier 2: Contains match on typeId (handles "MyObjectBuilder_Component" containing "Component") + exact subtypeId
        ...
        // Helper to decide inclusion based on sortId category toggles
        bool IncludeBySortId(string sortId)
        {
            if (string.IsNullOrEmpty(sortId)) return true; // unknown/misc keep
            string sid = sortId.ToLowerInvariant();
            if (sid == "component") return surfaceData.showComponents;
            if (sid == "protocomponent") return surfaceData.showProtoComponents;
            return true; // all other categories unaffected
        }
    }
}
